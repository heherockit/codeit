...
import { register } from '../../src/skills/trae/index.js';
...
describe('trae skill – analyze repositories (repo identification)', () => {
  ...
  it('populates affectedRepositories from CLI newline-separated output', async () => {
    ...
    await registry.resolve('analyze repositories with Trae')(ctx);
  ...
  it('passes a prompt containing the work item title and available repos to the CLI', async () => {
    ...
    const ctx = makeContext(
      ...
      { trae: { cliPath: 'trae' }, git: { workSpacePath: '/workspace' } },
    );
  ...
  it('uses "trae" as the default CLI executable', async () => {
    stubReaddir([]);
    stubExecFileOut('');
...
describe('trae skill – implement changes', () => {
  beforeEach(() => vi.clearAllMocks());
  ...
  it('passes an implementation prompt containing the work item and repo list to the CLI', async () => {
    ...
    await registry.resolve('implement changes with Trae')(ctx);
